...
<h3>修改床位</h3>
<form action="/BedServlet/update" method="post">

    <%--隐藏域，提交id--%>
    <input type="hidden" name="id" value="${bed.id}">

    房间号：<input name="roomId" value="${bed.roomId}"><br>
    所在建筑：<input name="buildingId" value="${bed.buildingId}"><br>
    使用者编号：<input name="userId" value="${bed.userId}"><br>

    是否可用
    <c:if test="${bed.isAvailable == 0}">

        <input type="radio" name="isAvailable" value="0" checked>禁用
    ...
    <c:if test="${bed.isAvailable == 1}">
        ...
        <input type="radio" name="isAvailable" value="1" checked>启用<br>
    </c:if>
