...
<table border="1" cellspacing="0" width="500">
    ...
    <c:forEach items="${nursinglevels}" var="nursinglevel" varStatus="nursinglevelstatus">
        <tr align="center">

            <td>${nursinglevelstatus.count}</td>
            <td>${nursinglevel.name}</td>

            <c:if test="${nursinglevel.status == 1}">
                <td>启用</td>
            </c:if>
            <c:if test="${nursinglevel.status != 1}">
                <td>禁用</td>
            ...
            <td><a href="/NursingLevelServlet/selectById?id=${nursinglevel.id}">修改</a>
                <a href="/NursingLevelServlet/deleteById?id=${nursinglevel.id}">删除</a></td>
        </tr>
    </c:forEach>
</table>
